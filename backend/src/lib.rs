...
pub fn api_router() -> Router<AppState> {
    let mut router = Router::new()
        ...
        .route(
            "/api/all-elections",
            get(routes::elections::list_all_elections),
        )
        .route(
            "/api/election-choices",
            get(routes::elections::get_election_choices),
        )
        .route(
            "/api/registration",
            get(routes::elections::get_registration),
        )
        .route(
            "/api/elections/dates",
    ...
    for config in STATE_SCRAPERS {
        ...
        router = router
            .route(
                ...
                axum::routing::post(
                    move |State(supabase): State<Arc<SupabaseClient>>| async move {
