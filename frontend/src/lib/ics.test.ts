...
describe("serializeIcs", () => {
  it("uses CRLF, escapes text, stable UID, and exclusive all-day end", () => {
    ...
    const first = serializeIcs([item]); expect(first).toBe(serializeIcs([item])); expect(first).toContain("DTSTART;VALUE=DATE:20261231"); expect(first).toContain("DTEND;VALUE=DATE:20270101"); expect(first).toContain("Caf\\é\\, deadline\\; now"); expect(first.endsWith("\r\n")).toBe(true);
  });
